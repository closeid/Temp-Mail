...
- refactor: |Frontend| Replace the Vue 3/Naive UI frontend with Vite, React, React Router, TanStack Query, Tailwind CSS, shadcn/ui-style components, and Radix UI while preserving existing APIs, authentication, localStorage contracts, and mail workflows. Redesign login, registration, inbox, sent mail, message reading, address management, user settings, appearance, admin, Telegram, and OAuth screens around a dense notes-workbench layout; standardize continuous desktop panes, mobile bottom navigation, responsive dialogs, form controls, loading states, and destructive-action dialogs; and add desktop, mobile, and dark-mode browser regression coverage
- refactor: |Frontend| Consolidate the user workspace into the root route and move the admin frontend route from `/admin` to `/dashboard` while keeping Worker `/admin/*` APIs unchanged; remove the global tab-placement preference, keep desktop navigation on the left, and fix user-bar vertical alignment, mailbox side gaps, full-height split handles, mobile control heights, and selected-state emphasis in the bottom menu
- fix: |Frontend| Continue aligning authentication, mailbox, user settings, and admin pages with the reference workbench framework: make loading overlays transparent, remove the initial homepage placeholder, normalize select, pagination, auto-refresh, and button heights, and fix pane scrolling, exact mailbox sizing, mobile fixed navigation, and nested tab layouts
- refactor: |Frontend| Rebuild the complete frontend visual system after auditing the live reference site's workspace, profile, member, import/export, AI, login-settings, and mobile pages. Login, registration, inbox, sent mail, message reading, user settings, admin, Telegram mail, and OAuth callbacks now share its cool-gray canvas, green primary actions, compact controls, and continuous columns; desktop uses a 244px primary rail with vertical nested settings navigation, mobile uses a bottom workspace menu, and a new envelope-plus brand mark replaces the old logo while preserving existing routes, APIs, and business behavior

- style: |Frontend| Align standard buttons, links, selected tabs, and informational states on the reference mailbox's neutral gray palette; remove the empty `n-tabs-pad` generated by card-style mailbox, user, and admin workspace tabs, drop `.app-shell` padding, and standardize regular modals on a 720px maximum with viewport-aware gutters
...
- refactor: |Frontend| Consolidate anonymous authentication on the home page with Login/Register tabs for user login, registration, password reset, email-address credentials, Passkey, and OAuth; switch the authenticated account workspace inside the root route from the home menu, with legacy `/user` URLs redirecting home. The authentication home no longer instantiates the Header or Footer, while site initialization and global access verification are extracted from the Header and the duplicate home/user login branches are removed
