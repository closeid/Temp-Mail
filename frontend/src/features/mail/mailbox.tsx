...
export function Mailbox({ queryKey, fetchMailData, deleteMail, canDelete, showEmailTo = true, showReply, showSaveS3, saveToS3, showFilter }: Props) {
  ...
  const list = <div role="list" className="h-full overflow-auto">{query.isLoading ? Array.from({ length: 7 }).map((_, index) => <div key={index} className="h-[76px] animate-pulse border-b border-border bg-transparent" />) : data.length ? data.map((mail) => <button type="button" role="listitem" key={mail.id} className={`grid w-full gap-1 border-b border-border px-3 py-2.5 text-left hover:bg-muted/50 ${selectedMail?.id === mail.id ? 'bg-accent' : ''}`} onClick={() => choose(mail)}>
    ...
    <span className="flex min-w-0 items-center gap-2 text-xs text-muted-foreground"><span className="min-w-0 flex-1 truncate">{showEmailTo ? 'FROM: ' : ''}{mail.source}</span>{showEmailTo && <span className="hidden max-w-36 truncate xl:block">TO: {mail.address}</span>}<span className="numeric">#{mail.id}</span></span>
