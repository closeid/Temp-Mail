  ...
  useEffect(() => {
    const run = async () => {
      try {
        ...
        const result = await api.fetch<{ jwt: string }>('/api/user/oauth2/callback', { method: 'POST', body: { code, state: params.get('state'), clientID: appStore.getState().userOauth2SessionClientID } })
