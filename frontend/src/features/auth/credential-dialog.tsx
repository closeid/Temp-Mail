...
export function CredentialDialog() {
  ...
  return <Dialog open={open} onOpenChange={(value) => appStore.setState({ showAddressCredential: value })}><DialogContent className="w-[min(720px,calc(100vw-32px))]"><DialogHeader><DialogTitle>{t('title')}</DialogTitle><DialogDescription>{t('tip')}</DialogDescription></DialogHeader>
    ...
    {state.openSettings.enableAgentEmailInfo && <details className="group border-t border-border pt-3"><summary className="cursor-pointer text-sm font-semibold">{t('agentAccess')}</summary><div className="mt-3 grid gap-3"><p className="text-sm text-muted-foreground">{t('agentAccessTip')}</p><CredentialField label={t('apiBase')} value={apiBase} /><CredentialField label={t('agentConfig')} value={agentConfig} /><Button variant="link" asChild><a href={`https://temp-mail-docs.awsl.uk/${locale === 'zh' ? 'zh' : 'en'}/guide/feature/agent-email.html`} target="_blank" rel="noreferrer">{t('docs')}<ExternalLink /></a></Button></div></details>}
