    ...
    {!canCreate ? <div className="rounded-md border border-border bg-muted p-3 text-sm text-muted-foreground">{t('createEmailUnavailable')}</div> : <>
      ...
      <Field label={t('address')}>
        <div className="flex min-w-0 items-stretch">
          {prefix && <span className="flex items-center rounded-l-md border border-r-0 border-input bg-muted px-2 text-sm text-muted-foreground">{prefix}</span>}
          <Input className="min-w-24 rounded-none first:rounded-l-md" value={openSettings.disableCustomAddressName ? t('autoGeneratedName') : name} disabled={openSettings.disableCustomAddressName} minLength={openSettings.minAddressLen} maxLength={openSettings.maxAddressLen} onChange={(event) => setName(event.target.value)} />
          <span className="flex items-center border-y border-input bg-muted px-2 text-muted-foreground">@</span>
          <Select value={domain} onValueChange={setDomain}><SelectTrigger className="min-w-36 rounded-l-none"><SelectValue /></SelectTrigger><SelectContent>{domains.map((item) => <SelectItem key={item.value} value={item.value}>{item.label}</SelectItem>)}</SelectContent></Select>
        </div>
      </Field>
