...
export function AccountTable() {
  ...
  return <><DataTablePage endpoint="/api/admin/address" columns={columns} actions={(row, refetch) => <Actions>
    <DropdownMenuItem onSelect={async () => { const jwt = await api.adminShowAddressCredential(row.id); setCredential({ address: row.name, jwt }) }}><KeyRound />{t('showCredential')}</DropdownMenuItem>
