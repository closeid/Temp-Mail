...
export function AccountTable() {
  ...
  return <><DataTablePage endpoint="/api/admin/address" columns={columns} actions={(row, refetch) => <Actions>
    ...
    <DropdownMenuItem disabled={!row.mail_count} onSelect={async () => { if (await confirmAction({ title: 'Clear inbox', description: row.name, destructive: true }) && await run(() => api.fetch(`/api/admin/clear_inbox/${row.id}`, { method: 'DELETE' }))) await refetch() }}>Clear inbox</DropdownMenuItem>
    <DropdownMenuItem disabled={!row.send_count} onSelect={async () => { if (await confirmAction({ title: 'Clear sent mail', description: row.name, destructive: true }) && await run(() => api.fetch(`/api/admin/clear_sent_items/${row.id}`, { method: 'DELETE' }))) await refetch() }}>Clear sent mail</DropdownMenuItem>
...
export function CreateAddressPage() {
  ...
  const create = async () => { if (!name || !domain) return toast.error('Complete all fields'); try { setResult(await api.fetch('/api/admin/new_address', { method: 'POST', body: { enablePrefix: prefix, enableRandomSubdomain: random, name, domain } })); toast.success('Address created') } catch (error) { toast.error(stringifyError(error)) } }
...
export function UserTable() {
  ...
  const create = async () => { if (!email || !password) return; const hashed = await hashPassword(password); if (await run(() => api.fetch('/api/admin/users', { method: 'POST', body: { email, password: hashed } }), 'User created')) { setCreateOpen(false); setEmail(''); setPassword(''); await client.invalidateQueries({ queryKey: ['admin-table', '/api/admin/users'] }) } }
...
export function SenderAccessPage() {
  ...
  return <><DataTablePage endpoint="/api/admin/address_sender" queryName="address" columns={columns} actions={(row, refetch) => <Actions><DropdownMenuItem onSelect={() => { setEditing({ ...row, refetch }); setEnabled(Boolean(row.enabled)); setBalance(Number(row.balance || 0)) }}>Modify</DropdownMenuItem><DropdownMenuItem className="text-destructive" onSelect={async () => { if (await confirmAction({ title: 'Delete sender access', description: row.address, destructive: true }) && await run(() => api.fetch(`/api/admin/address_sender/${row.id}`, { method: 'DELETE' }))) await refetch() }}><Trash2 />Delete</DropdownMenuItem></Actions>} />
  <Dialog open={Boolean(editing)} onOpenChange={(value) => !value && setEditing(null)}><DialogContent><DialogHeader><DialogTitle>Sender access</DialogTitle><DialogDescription>{editing?.address}</DialogDescription></DialogHeader><label className="flex items-center justify-between text-sm font-medium"><span>Enabled</span><Switch checked={enabled} onCheckedChange={setEnabled} /></label><Field label="Balance"><Input type="number" min={0} max={1000} value={balance} onChange={(event) => setBalance(Number(event.target.value))} /></Field><DialogFooter><Button onClick={async () => { if (!editing) return; if (await run(() => api.fetch('/api/admin/address_sender', { method: 'POST', body: { address: editing.address, address_id: editing.id, balance, enabled: enabled ? 1 : 0 } }))) { await editing.refetch(); setEditing(null) } }}><Save />Save</Button></DialogFooter></DialogContent></Dialog></>
}
...
export function MaintenancePage() {
  ...
  return <ObjectSettings endpoint="/api/admin/auto_cleanup" title="Automatic cleanup" description="Configure scheduled cleanup policies." extraActions={<Button variant="secondary" onClick={cleanup}><Trash2 />Cleanup now</Button>} />
}
...
export function TelegramAdminPage() {
  ...
  const init = async () => { await run(() => api.fetch('/api/admin/telegram/init', { method: 'POST' }), 'Telegram initialized'); await status.refetch() }
  return <ObjectSettings endpoint="/api/admin/telegram/settings" title="Telegram" extraActions={<><Button variant="secondary" onClick={() => status.refetch()}>Status</Button><Button variant="secondary" onClick={init}>Initialize</Button></>} description={status.data ? JSON.stringify(status.data) : 'Configure the Telegram mini app and push rules.'} />
}
