...
export function TelegramAddressPage() {
  ...
  const query = useQuery({ queryKey: ['telegram-addresses', initData], queryFn: () => api.fetch<any[]>('/telegram/get_bind_address', { method: 'POST', body: { initData } }), enabled: Boolean(initData) })
  const bind = () => api.fetch('/telegram/bind_address', { method: 'POST', body: { initData, jwt: appStore.getState().jwt } })
  const create = (name: string, domain: string, token: string, random: boolean) => api.fetch('/telegram/new_address', { method: 'POST', body: { initData, address: `${name}@${domain}`, cf_token: token, enableRandomSubdomain: random } })
  const choose = (jwt: string) => { appStore.setState({ jwt }); location.reload() }
  const unbind = async (address: string) => { if (!(await confirmAction({ title: 'Unbind address', description: address, destructive: true }))) return; try { await api.fetch('/telegram/unbind_address', { method: 'POST', body: { initData, address } }); await query.refetch(); toast.success('Address unbound') } catch (error) { toast.error(stringifyError(error)) } }
