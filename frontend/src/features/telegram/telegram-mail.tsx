...
export function TelegramMailPage() {
  ...
  const query = useQuery({ queryKey: ['telegram-mail', mailId, initData], queryFn: async () => processItem(await api.fetch('/telegram/get_mail', { method: 'POST', body: { initData, mailId } })), enabled: Boolean(mailId) })
