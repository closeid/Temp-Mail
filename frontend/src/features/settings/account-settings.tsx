...
import { DatabaseZap, KeyRound, ShieldAlert, Trash2 } from 'lucide-react'
...
import { AddressCredentialDetails } from '@/features/auth/credential-dialog'
...
export function AddressAccountSettings() {
  ...
  const [confirmAction, setConfirmAction] = useState<null | 'delete' | 'inbox' | 'sent'>(null)
  ...
  const run = async () => {
    try {
      ...
      if (confirmAction === 'delete') navigate(getPathWithLocale('/', locale))
  ...
  const labels = { delete: [t('deleteAccount'), t('deleteAccountConfirm')], inbox: [t('clearInbox'), t('clearInboxConfirm')], sent: [t('clearSentItems'), t('clearSentItemsConfirm')] } as const
  return <SettingsLayout>
    <AddressCredentialDetails />
    <Separator />
    <div className="grid gap-2">{openSettings.enableAddressPassword && <Button className="justify-start" variant="secondary" onClick={() => setPasswordOpen(true)}><KeyRound />{t('changePassword')}</Button>}{openSettings.enableUserDeleteEmail && <><Button className="justify-start" variant="secondary" onClick={() => setConfirmAction('inbox')}><DatabaseZap />{t('clearInbox')}</Button><Button className="justify-start" variant="secondary" onClick={() => setConfirmAction('sent')}><DatabaseZap />{t('clearSentItems')}</Button></>}</div>
