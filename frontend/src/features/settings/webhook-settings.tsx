...
export function WebhookSettings({ fetchData, saveSettings, testSettings }: { fetchData: () => Promise<any>; saveSettings: (model: Model) => Promise<any>; testSettings: (model: Model) => Promise<any> }) {
  const { t } = useScopedI18n('components.WebhookComponent')
  const [model, setModel] = useState(blank)
  const [available, setAvailable] = useState(true)
  useEffect(() => { fetchData().then((result) => setModel({ ...blank, ...result })).catch(() => setAvailable(false)) }, [])
  ...
  if (!available) return <div className="grid h-full place-items-center text-muted-foreground">{t('notEnabled')}</div>
