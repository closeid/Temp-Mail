...
export function UserSettingsPage() {
  ...
  const passkeys = useQuery({ queryKey: ['passkeys'], queryFn: () => api.fetch<any[]>('/user_api/passkey'), enabled: listOpen })
  const create = async () => { try { const options = await api.fetch<any>('/user_api/passkey/register_request', { method: 'POST', body: { domain: location.hostname } }); const credential = await startRegistration({ optionsJSON: options }); await api.fetch('/user_api/passkey/register_response', { method: 'POST', body: { origin: location.origin, passkey_name: name || `${navigator.platform || 'Unknown'}: ${Math.random().toString(36).slice(7)}`, credential } }); toast.success(t('passkeyCreated')); setName(''); setCreateOpen(false); if (listOpen) await passkeys.refetch() } catch (error) { toast.error(stringifyError(error)) } }
  const rename = async () => { try { await api.fetch('/user_api/passkey/rename', { method: 'POST', body: { passkey_name: name, passkey_id: current.passkey_id } }); await passkeys.refetch(); setRenameOpen(false); setName('') } catch (error) { toast.error(stringifyError(error)) } }
  const remove = async (id: string) => { if (!(await confirmAction({ title: t('deletePasskey'), destructive: true }))) return; try { await api.fetch(`/user_api/passkey/${id}`, { method: 'DELETE' }); await passkeys.refetch() } catch (error) { toast.error(stringifyError(error)) } }
