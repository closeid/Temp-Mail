...
- feat: |后台密码重置| 管理员点击“生成密码”后自动生成并填充 12 位安全密码、复制到系统剪贴板并提示复制成功；仅生成的密码会在输入框中直接显示，手动编辑后立即恢复密码掩码
- feat: |用户密码安全| 注册、找回密码、账号改密及后台创建/重置用户统一要求密码不少于 8 位且包含英文大小写、数字和符号；所有密码请求均由浏览器先生成 SHA-256 派生值，Worker 拒绝非 64 位十六进制校验值并在落库前进行 HMAC 保护；后台重置密码弹窗新增密码学安全的 12 位密码生成按钮
...
- feat: |后台安全| 新增“用户 -> 访问令牌”，支持生成、命名、设置过期时间和删除具有全部后台 API 权限的令牌，令牌明文仅显示一次且数据库仅保存哈希
- feat: |认证页面| 确认登录页会根据后台 OAuth2 配置自动显示提供商登录按钮，并新增桌面与移动端可视回归截图
- feat: |后台管理| “全部地址”操作菜单支持将未绑定邮箱归属到已注册用户；管理员可输入用户邮箱实时查询并选择目标账号，绑定成功后同步刷新地址列表与用户地址统计
- feat: |会话权限| 账号会话访问任一邮箱 JWT 时保留多邮箱切换能力，并可将当前无归属邮箱绑定到登录账号；独立邮箱 JWT 会话严格限制为当前邮箱，不显示地址管理或用户设置，也不能创建新邮箱
- feat: |后台管理/API| 创建邮箱地址时可选择无归属或通过用户邮箱查询并绑定已注册用户；`POST /api/admin/new_address` 新增可选 `ownerUserEmail` / `ownerUserId`，绑定失败会回滚新地址并遵守用户角色地址上限
- feat: |邮箱凭据登录| 增加可持久化的受限凭据会话：隐藏地址管理入口、顶部仅显示本地地址且禁止新建邮箱；未使用用户账号密码登录时，不可查看归属用户的其他邮箱
...
- fix: |Tag Build CI| 移除 WASM 邮件解析器构建中的动态依赖安装和源码补丁，改用固定依赖与独立解析器构建入口，并固定 Wrangler 版本以避免无锁安装时出现 peer dependency 不匹配
- fix: |国际化| 修复“地址凭证与连接方式”中 Address JWT 的中文标签显示
- fix: |前端提示与邮件列表| 将 Sonner 提示关闭按钮统一移动到右下角，并移除前台、后台收件箱和发件箱中所有邮件 ID 的可见展示
- fix: |后台管理| 用户已绑定地址弹窗中的创建时间在桌面端与移动端保持单行显示，空间不足时由表格容器横向滚动
- fix: |前端会话与页首| 后台管理员密码保持纯内存状态且仅使用 Worker 配置，刷新或退出后立即清除；前后台退出登录统一移到页面右上角，设置页移除重复入口；移动端邮箱页首操作仅显示图标并保持单行
- fix: |认证安全| OAuth2 登录状态改用加密安全随机值，并在回调后立即清理会话及移动端兼容状态；登录表单支持在邮箱或密码输入框按回车提交；邮件 AI 提取链接仅允许打开 HTTP/HTTPS 地址；升级 DOMPurify 与 AWS S3 SDK，移除已知的 HTML 清洗和 XML 解析依赖漏洞
- fix: |页面标题| 邮箱工作区按“页面 - 邮箱地址 - Get an Email”显示标题，后台仅显示“后台管理 - Get an Email”，不再继承邮箱地址
- fix: |认证页面| 后台关闭新用户注册后，登录页只显示登录页签与登录表单，不再渲染注册入口；直接访问注册路由会自动返回登录页

- fix: |前端| 地址凭据与连接方式改为在邮箱设置页直接显示；删除用户设置页的退出按钮，其他退出操作统一跳转根路由，并使用当前邮箱地址作为页面标题
- fix: |后台管理| 将自动清理配置重构为天数输入框与右侧开关的逐项布局，自定义 SQL 使用独立名称、SQL 和启用控件，并提供可直接参考的 DELETE 示例
- fix: |国际化| 前端与文档语言精简为中文和 English，移除西班牙文、葡萄牙文、日文和德文运行时字典及路由识别
- fix: |后台管理| 邮箱地址列表增加所属用户信息，并明确标记未绑定的匿名邮箱；用户列表将地址数量明确为已绑定地址数，避免把全部邮箱数量与用户绑定数量错误对比
- fix: |认证路由| 删除旧 `/address-login` 路由，将忘记密码调整为 `/login/forgot-password`，新增独立后台登录页 `/dashboard/login`，未认证访问任意后台页面时自动跳转并在登录后返回原页面
- fix: |认证| 将密码登录、邮箱凭据登录和 Passkey 登录统一保留在 `/login`；用户取消或超时关闭 Passkey 提示时不再显示错误，并在登录、注册、找回密码、发送验证码及 Worker 认证接口中统一校验用户邮箱格式
...
- refactor: |后台导航| 将统计总览提升为 `/dashboard` 首页并新增一级“首页”入口，未登录访问时跳转 `/dashboard/login`；配置菜单移除统计二级页面，邮箱归属用户操作移除前置图标

- fix: |后台管理| 修改用户角色改用 `USER_ROLES` 下拉选择，并提供恢复默认角色选项，避免输入不存在的角色名
- refactor: |前端路由| 为登录、注册、忘记密码、地址凭据登录、收件箱、发件箱、写信、地址管理、全部设置页及后台全部二级页面建立显性 URL，菜单、OAuth 回调、邮件回复与地址切换统一通过路由注册表导航；移除 `indexTab`、`adminTab` 等会话菜单状态，并为 Cloudflare Pages 增加 SPA 深层路径刷新回退
- fix: |后台管理| 角色地址配置在 `USER_ROLES` 为空时仍提供可增删的角色限额表单，并合并显示已保存配置；OAuth2 登录配置空状态不再渲染重复的粗分隔线
- fix: |后台管理| 地址凭证操作移除多余图标，创建地址名称、`@` 分隔块与域名选择框统一为 40px；地址栏与地址管理复用同一绑定地址查询并在绑定后失效刷新，避免登录用户看到本地缓存的未绑定地址
- fix: |后台管理| 密码登录改为用户名加密码（默认用户名为 `admin`，可由 `ADMIN_USERNAME` 覆盖），未认证时不再渲染后台内容；自动清理配置在首次使用时提供完整安全默认值，IP 黑名单和地址规则文本项提供示例占位符
- security: |前后端| 后台密码不再持久化到浏览器本地存储，按 API 范围附带认证头，隐藏 Worker 内部异常文本；限制 IP/ASN/浏览器指纹规则；结构化构造 OAuth2 授权 URL；校验 Webhook 协议、目标、方法、请求头和负载并禁止私网目标；密码登录入口纳入现有速率限制
- docs: |配置| 补充 `ADMIN_USERNAME` 的 Worker 模板、README 与中英文变量文档说明
- fix: |前端| 补齐创建邮箱表单“邮箱地址”字段的六语言翻译，避免回退显示 `address`；统一邮箱名称输入框、`@` 分隔块与域名下拉框为 40px 高度
...
- docs: |后台管理| 补充角色地址上限的继承规则、OAuth2 与密码登录的关系、Webhook 总开关行为，以及发信凭据的安全配置位置

- refactor: |Worker| 所有 HTTP API 统一收束到 `/api` 命名空间，公开、用户、后台、Telegram 与外部发信接口分别使用 `/api/open/*`、`/api/user/*`、`/api/admin/*`、`/api/telegram/*` 与 `/api/external/*`；删除 Worker 内部旧路径兼容转发，并同步更新 Pages 代理与中英文文档
- refactor: |前端| 普通用户工作区收束为收件箱、邮件、地址管理和设置四个入口，删除重复的“绑定邮箱地址”菜单；后台一级导航收束为四项，并新增完整 API 文档页
- fix: |前端| 补齐新版 React 后台与共享组件的六语言翻译，覆盖表格、操作菜单、弹窗、数据库维护、自动清理、Telegram、动态设置字段和分页控件

- refactor: |前端| 在保持现有 API、认证、本地存储与邮件业务流程不变的前提下，将前端从 Vue 3/Naive UI 全面迁移至 Vite、React、React Router、TanStack Query、Tailwind CSS、shadcn/ui 风格组件与 Radix UI；按高密度笔记工作台重新设计登录、注册、收件箱、发件箱、邮件阅读、地址管理、用户设置、外观、管理后台、Telegram 与 OAuth 页面，统一桌面连续分栏、移动底栏、响应式弹窗、表单控件、加载状态及危险操作对话框，并新增桌面/移动/暗色模式浏览器回归测试
- refactor: |前端| 将用户工作区收束到根路由，并将管理后台前端路由由 `/admin` 迁移至 `/dashboard`（Worker `/admin/*` API 保持不变）；移除外观中的全局页签位置设置并固定桌面左侧导航，修复当前用户栏垂直居中、邮件列表左右留白、全高分栏拖拽线、移动端控件高度和底部菜单选中强调
- fix: |前端| 继续按参考站点的工作台框架统一认证、邮箱、用户设置和管理后台页面：加载遮罩改为透明并移除首页初始化占位，统一选择框、分页器、自动刷新与按钮高度，修复内容区滚动、邮箱精确高度、移动端固定导航和二级页签布局
- refactor: |前端| 登录并审阅真实参考站点的工作区、个人设置、成员、导入导出、AI、登录设置及移动端页面后，按其高密度笔记工作台重构全站视觉系统：统一登录、注册、邮箱、发件箱、邮件阅读、用户设置、管理后台、Telegram 邮件与 OAuth 回调页的冷灰画布、绿色主操作、紧凑控件和连续分栏；桌面端采用 244px 主侧栏与纵向二级设置导航，移动端使用底部工作区菜单，并用新的信封加号品牌标记替换旧 Logo，同时保留现有路由、API 与业务功能
...
- style: |前端| 精简首页认证品牌区，仅保留放大的 Logo；登录与注册页签统一使用 600 字重，将忘记密码入口居中对齐，并设置为 30px 高、左右 10px 内边距
- refactor: |前端| 将未登录认证统一收敛到首页，以“登录/注册”页签整合用户登录、注册、忘记密码、邮箱地址凭据、Passkey 和 OAuth 登录；登录后的账户工作区通过首页菜单在根路由内切换，旧 `/user` 地址兼容重定向到首页。认证首页不再实例化页首与页脚，并抽离站点初始化与全局访问验证，删除旧首页和用户页的重复登录分支
- fix: |前端| 修复邮箱地址凭据登录页的注册邮箱入口被隐藏的问题，移除帮助页签与页首导航中的 GitHub/版本链接，并将邮箱地址凭据登录页统一为无页首、无页脚的独立认证布局；服务端创建邮箱权限校验与原有登录、注册流程保持不变
- style: |前端| 将应用名称统一为 `Get an Email`，登录与注册页移除页首导航，并按参考图重构收件箱、发件箱、邮件阅读区、设置导航、浅色与深色配色及移动端布局，保留现有 API 与业务流程
