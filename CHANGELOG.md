...
- feat: |后台安全| 管理员密码会话可在后台修改根管理员密码，D1 仅保存 SHA-256 哈希并覆盖部署变量；新增“用户 -> 访问令牌”，支持生成、命名、设置过期时间和删除具有全部后台 API 权限的令牌，令牌明文仅显示一次且数据库仅保存哈希
...
- fix: |前端会话与页首| 后台管理员凭据改为当前标签页会话存储，刷新不再要求重新登录；前后台退出登录统一移到页面右上角，设置页移除重复入口；移动端邮箱页首操作仅显示图标并保持单行
