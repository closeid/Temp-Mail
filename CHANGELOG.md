...
- fix: |前端| 继续按参考站点的工作台框架统一认证、邮箱、用户设置和管理后台页面：加载遮罩改为透明并移除首页初始化占位，统一选择框、分页器、自动刷新与按钮高度，修复四向全局页签菜单、内容区滚动、邮箱精确高度、移动端固定导航和二级页签布局
...
- refactor: |前端| 将未登录认证统一收敛到首页，以“登录/注册”页签整合用户登录、注册、忘记密码、邮箱地址凭据、Passkey 和 OAuth 登录；`/user` 仅保留登录后的账户工作区，未登录访问会返回首页。认证首页不再实例化页首与页脚，并抽离站点初始化与全局访问验证，删除旧首页和用户页的重复登录分支
