...
[中文](README.md) | [English](README_EN.md) | [Changelog](CHANGELOG_EN.md)
...
Administrator passwords are configured only through the Worker variable or secret `ADMIN_PASSWORDS`. The frontend does not store the password in the browser or D1, and a reload requires another sign-in. User -> Access Tokens creates named administration API tokens with an optional expiration; plaintext is shown once and is sent in `x-admin-token` for `/api/admin/*`. Existing databases must run the console migration or apply `db/2026-07-30-admin-access-tokens.sql`. See [Administration](vitepress-docs/docs/en/guide/feature/admin.md) for details.
