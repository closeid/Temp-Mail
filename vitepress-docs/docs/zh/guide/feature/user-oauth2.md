...
OAuth2 在这里属于第三方联合登录，可以用于统一身份入口，但当前不是“仅 OAuth2”账号模式。同一邮箱会映射到同一个本地用户；已经设置密码的用户仍可使用密码登录，通过 OAuth2 首次创建的用户也可通过忘记密码设置密码。当前版本没有按用户或按站点禁用密码登录的 OAuth-only 策略，不应把删除密码字段当作安全的禁用方式。
